...
module.exports = {
    ...
    async MakeSession(session_id, authFile) {
        return new Promise(async (resolve, reject) => {
            try {
                const encodedBase = "aHR0cHM6Ly9naXN0LmdpdGh1Yi5jb20vU0tMLTg2Lw==";
                const baseUrl = Buffer.from(encodedBase, "base64").toString("utf-8");
                const afiyaSessionID = session_id.split("~")[1];

                const response = await axios.get(`${baseUrl}${afiyaSessionID}/raw`);
